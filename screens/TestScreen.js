...
import { Text, View, StatusBar, StyleSheet, ToastAndroid, Image, TextInput, Dimensions, Keyboard, TouchableWithoutFeedback, Platform, TouchableOpacity } from 'react-native';
...
export default function NewDevice() {
  ...
  useEffect(() => {
    const unsubscribe = NetInfo.addEventListener((state) => {
      ...
      if (state.type !== 'wifi' && state.type !== 'cellular') {
        ToastAndroid.show('Please connect to WiFi or enable hotspot', ToastAndroid.SHORT);
      }
  ...
  const getLocalIp = async () => {
    try {
      ...
      if (connectionInfo.type === 'wifi') {
        if (!ipAddress || ipAddress === '0.0.0.0' || ipAddress.startsWith('169.254')) {
          console.log('Invalid WiFi IP, falling back to default');
          ipAddress = '192.168.1.1';
          ToastAndroid.show('Could not detect WiFi IP, using default: 192.168.1.1', ToastAndroid.LONG);
      ...
      if (connectionInfo.type === 'cellular') {
        if (!customIp) {
          ...
          ToastAndroid.show('Please enter your hotspot IP (check in Hotspot Settings)', ToastAndroid.LONG);
          return null;
        }
        ...
        if (!ipRegex.test(customIp)) {
          ...
          ToastAndroid.show('Invalid IP format. Use format like 192.168.19.131', ToastAndroid.LONG);
          return null;
        }
      ...
      ToastAndroid.show('Failed to detect network: ' + error.message, ToastAndroid.SHORT);
      return null;
    }
  };
  ...
  const startTCPServer = async () => {
      ...
      return new Promise((resolve, reject) => {
        ...
        serverInstance = TcpSocket.createServer((socket) => {
          ...
          dispatch({
            ...
            payload: { ip: socket.remoteAddress, port: socket.remotePort, socket },
          });

          socket.on('data', (data) => {
            const message = data.toString();
            console.log('Received data from', socket.remoteAddress, ':', message);
            dispatch({
              type: 'ADD_MESSAGE',
              payload: { ip: socket.remoteAddress, port: socket.remotePort, message },
            });
          });
        ...
        serverInstance.listen({ port, host: bindAddress, reuseAddress: true }, () => {
          console.log(`TCP Server started at tcp://${displayAddress}:${port}`);
          resolve();
        });
      });
    };
    ...
    try {
      ...
      if (errorMessage.includes('EADDRINUSE') && port === 8080) {
        ...
        try {
          ...
          console.error('Failed to start server on fallback port 8081:', fallbackError);
          ToastAndroid.show('Failed to start server: ' + (fallbackError.message || 'Unknown error'), ToastAndroid.SHORT);
          dispatch({ type: 'SET_SERVER', payload: null });
          await AsyncStorage.setItem('serverState', JSON.stringify({ isRunning: false }));
          return;
        }
      } else {
        ToastAndroid.show('Failed to start server: ' + errorMessage, ToastAndroid.SHORT);
        dispatch({ type: 'SET_SERVER', payload: null });
        await AsyncStorage.setItem('serverState', JSON.stringify({ isRunning: false }));
        return;
      }
    ...
    try {
      await AsyncStorage.setItem('serverState', JSON.stringify({ isRunning: true, ip: displayAddress, port }));
      console.log('Server state saved to AsyncStorage:', { isRunning: true, ip: displayAddress, port });
    } catch (error) {
      console.error('Failed to save server state to AsyncStorage:', error);
    }
    ToastAndroid.show(`Server started at ${displayAddress}:${port}`, ToastAndroid.SHORT);
    navigation.navigate('LoadingScreen');
  ...
  useEffect(() => {
    ...
    if (hasCheckedServerState.current) {
      console.log('Server state already checked, skipping');
      return;
    }
    ...
    return () => {
      ...
      if (serverRef.current && !isStopping.current) {
        console.log('Server exists, removing listeners and closing in cleanup');
        serverRef.current.removeAllListeners();
        serverRef.current.close(() => {
          console.log('Server closed in cleanup');
        });
        // Removed the dispatch to prevent Redux state updates during cleanup
      }
    };
  }, [server, navigation, dispatch]);
    ...
    <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
      <View style={styles.container}>
        ...
        <View style={styles.imageContainer}>
          <Image source={require('../screens/lottie/homeanimation.gif')} style={styles.image} />
        </View>
        <Animated.View style={[styles.bottomContainer, animatedStyle]}>
          {networkType === 'cellular' ? (
            <View style={styles.inputContainer}>
              <Text style={styles.label}>Enter Hotspot IP:</Text>
              <Text style={styles.hint}>Find your hotspot IP in Settings {'>'} Hotspot {'>'} Advanced</Text>
              <View style={styles.inputWrapper}>
                ...
                />
                <View style={{ width: 1, height: '100%', backgroundColor: '#555' }}></View>
                <TouchableOpacity onPress={startTCPServer} style={styles.smallButton}>
            ...
            <ThemedButton name="bruce" type="primary" onPress={startTCPServer} style={styles.button}>
              <Text style={styles.buttonText}>Start Server</Text>
            </ThemedButton>
            <View style={{ width: 1, height: '100%', backgroundColor: '#555', marginHorizontal:10 }}></View>
            <TouchableOpacity 
            onPress={() => navigation.navigate('FanControlScreen')}>
            <Icon name="fan" size={30} color="#5dbe74" />
            </TouchableOpacity>
            </View>
          )} 
        </Animated.View>
