        ...
        serverInstance = TcpSocket.createServer(socket => {
          ...
          dispatch({
            ...
            payload: {
              ...
              port: Number(socket.remotePort), // Ensure port is a number
              socket,
            },
          });

          const buffers = new Map(); // Store buffers for each client

          // Simplified data handler to confirm the event is firing
          socket.on('data', data => {
            console.log('### DATA EVENT TRIGGERED ###');
            console.log('Received data from:', socket.remoteAddress, 'at:', new Date().toISOString());
            console.log('Data (raw):', data);
            console.log('Data type:', typeof data, 'Is Buffer?', Buffer.isBuffer(data));

            if (!Buffer.isBuffer(data)) {
              console.error('Data is not a Buffer, dispatching as text');
              dispatch({
                type: 'ADD_MESSAGE',
                payload: {
                  ip: socket.remoteAddress,
                  ...
                  type: 'text',
                  message: 'Error: Received invalid data (not a Buffer)',
                },
              });
              return;
            }
            ...
            let buffer = buffers.get(clientKey) || Buffer.alloc(0);
            buffer = Buffer.concat([buffer, data]);
            console.log('Current buffer length:', buffer.length);

            if (buffer.length < 3) {
              console.log('Buffer too small to check for JPEG, buffering more data');
              buffers.set(clientKey, buffer);
              return;
            }

            const firstThreeBytes = buffer.slice(0, 3).toString('hex').toLowerCase();
            console.log('First three bytes (hex):', firstThreeBytes);
            const isJPEG = firstThreeBytes === 'ffd8ff';
            console.log('isJPEG check:', isJPEG);

            if (!isJPEG) {
              console.log('Not a JPEG, dispatching as text');
              dispatch({
                type: 'ADD_MESSAGE',
                payload: {
                  ip: socket.remoteAddress,
            ...
            if (endIndex !== -1) {
              const jpegData = buffer.slice(0, endIndex);
              console.log('Detected complete JPEG data, size:', jpegData.length);
              ...
              const remainingData = buffer.slice(endIndex);
              buffers.set(clientKey, remainingData);
              if (remainingData.length > 0) {
                console.log('Processing remaining data:', remainingData.length, 'bytes');
                socket.emit('data', remainingData);
              } else {
                buffers.delete(clientKey);
              }
            } else {
              console.log('Buffering incomplete JPEG data, current length:', buffer.length);
              buffers.set(clientKey, buffer);
            }
          });
          socket.on('close', () => {
