...
const REFRESH_TOKEN =
  'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9..sAKpC_xzkRrnude0uDoiDvYHo6dz_s0L10m5jhgJin4';
...
export default function FanControlScreen({navigation}) {
  ...
  useEffect(() => {
    const storeApiCredentials = async () => {
      try {
        ...
        ToastAndroid.show(
          'Failed to store API credentials',
          ToastAndroid.SHORT,
        );
      }
    };
  ...
  const getValidAccessToken = async () => {
    ...
    console.log('Stored access token and timestamp:', {
      storedAccessToken,
      timestamp,
    });

    const currentTime = Date.now();
    const tokenAge = timestamp
      ? (currentTime - parseInt(timestamp)) / 1000
      : Infinity;
  ...
  const fetchDevices = async () => {
    try {
      ...
      const response = await axios.get(
        `${API_BASE_URL}/v1/get_list_of_devices`,
        {
          headers: {
            'x-api-key': apiKey,
      ...
      {rateLimitExceeded ? (
        <View style={styles.errorContainer}>
          ...
          <Text style={styles.errorText}>
            API Rate Limit Exceeded. Please wait until the limit resets (usually
            24 hours) or contact Atomberg support.
          </Text>
        </View>
      ) : selectedDeviceId ? (
        fanState ? (
              ...
              <Animatable.View
                ...
                <TouchableOpacity
                  ...
                  <LinearGradient
                    ...
                    <Icon name="power-off" size={width * 0.06} color="#fff" />
                  </LinearGradient>
                </TouchableOpacity>
              ...
              <Animatable.View
                ...
                <TouchableOpacity
                  style={[
                    ...
                    (!fanState.is_online ||
                      !fanState.power ||
                      fanState.last_recorded_speed <= 1) &&
              ...
              <Animatable.View
                ...
                <TouchableOpacity
                  style={[
                    ...
                    (!fanState.is_online ||
                      !fanState.power ||
                      fanState.last_recorded_speed >= 6) &&
                      styles.buttonDisabled,
                  ]}
                  onPress={() => sendCommand({speedDelta: 1})}
                  disabled={
